...
class Map3D{
  ...
  setCameraPosition(v3,time,delay,easing,callback){
    if(time && typeof time==='number')
      Map3D.transition(this.camera.position,v3,time,delay,easing,callback);
    else
   ...
   */
  static transition(from,to,time,delay,easing,callback){
    if(typeof time !=='number'){
      time=1000;
    }
    if(!callback)callback=()=>{};
...
class Area extends THREE.Object3D{
  ...
  getGeoMesh(geo,pros){
    ...
    //   uniforms: THREE.BokehShader.uniforms,
    ...
    // var texture= new THREE.TextureLoader().load('static/wl.png');
    // texture.wrapS = THREE.RepeatWrapping;
    // texture.wrapT = THREE.RepeatWrapping;
    // //texture.needsUpdate = true;
    // var material= new THREE.MeshPhongMaterial(Object.assign(mateOption,{map: texture,color:0xffffff,overdraw:true}));

    // var materialFront = new THREE.MeshLambertMaterial({ map:texture, color: 0xcceeff });
    // var materialSide = new THREE.MeshLambertMaterial({  color: 0x333333});
    ...
    if(this.userData.hasPhong)
      //geoMesh = new THREE.Mesh(geo,shaderMaterial);
     // geoMesh = new THREE.Mesh(geo, objectMaterial)//new THREE.MeshPhongMaterial(mateOption));
  ...
  setColor(color,time,delay,easing,callback){
    ...
    if(time && typeof time==='number'){
      ...
      Map3D.transition(this.mesh.material.color,color,time,delay,easing,callback);
    }
    else {
  ...
  setPosition(v3,time,delay,easing,callback){
    if(time && typeof time==='number')
      Map3D.transition(this.position,v3,time,delay,easing,callback);
    else
  ...
  setRotation(v3,time,delay,easing,callback){
    ...
    Map3D.transition(this.rotation,v3,time,delay,easing,callback);
  }
  /**
   * 设置区域大小
   * @param {v3} v3 - 格式{x:0,y:0,z:0}
  ...
  setScale(v3,time,delay,easingcallback){
    if(time && typeof time==='number')
      Map3D.transition(this.scale,v3,time,delay,easing,callback);
    else
...
class Mark extends THREE.Sprite{
  ...
  setColor(color, time, delay, easing,callback){
    ...
    if(time && typeof time==='number'){
      color=new THREE.Color(colorToHex(color));
      Map3D.transition(this.material.color,color,time,delay,easing,callback);
    }
    else {
