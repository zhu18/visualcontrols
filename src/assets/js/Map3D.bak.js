 ...
 * 创建3D地图.
 * @class Map3D
 * @author 九次方前端研发部朱润亚 <zhu18@vip.qq.com>
 * @example
...
export default class Map3D{
  ...
  constructor(o){
    ...
    /**
     * @lends Map3D
     * @type {{name: string, el: HTMLElement, geoData: null, hasStats: boolean, hasControls: boolean, autoRotate: boolean, ambientColor: number, directionalColor: number, hasLoadEffect: boolean, debugger: boolean, cameraPosition: {x: number, y: number, z: number}, visualMap: null, extrude: {amount: number, bevelThickness: number, bevelSize: number, bevelEnabled: boolean, bevelSegments: number, curveSegments: number, steps: number}, area: {data: Array, color: number, hoverColor: number, lineColor: number, opacity: number, hasPhong: boolean, shininess: number, hoverAnimaTime: number, loadEffect: boolean, hasHoverHeight: boolean}, mark: {data: Array, color: number, hoverColor: number, hoverAnimaTime: number, min: number, max: number}, line: {data: Array, color: number, hoverColor: number, hoverExclusive: boolean, hoverAnimaTime: number, spaceHeight: number, hasHalo: boolean, hasHaloAnimate: boolean, haloDensity: number, haloRunRate: number, haloColor: number, haloSize: number, decayColor: number}, mapObject: null, scene: null, camera: null, renderer: null, stats: null, controls: null, areaCount: number, _w: number, _h: number, __event: null}}
     */
    let opt={
      /*外部参数*/
