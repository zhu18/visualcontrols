...
export default class Map3D{
  constructor(o){
    ...
    let opt={
      ...
      areaData:[],        //地图用户数据[{name:'北京',value:,color:0xff3333}...]
      markData:[],        //标注点数据[{name:'XXX',coord:[11,22],value:13}...]
      debugger:false,     //调试模式
  ...
  init(){
    ...
    this.geoData.features.forEach((geo)=>{
      this.markData.push({name:geo.properties.name,coord:geo.properties.cp,value:1})
    })
  ...
  initArea(){
    ...
    this.geoData.features.forEach((item)=>{
      //地图属性 & 用户属性合并
      let itemUserData = this.areaData.find(val=> val.name===item.properties.name );
  ...
  initMark(){
    ...
    this.markData.forEach((data)=>{
      Object.assign(pros,data);
      let mark = new Mark(pros);
...
class Mark extends THREE.Sprite{
  ...
  constructor(pros){
    ...
    this.material = new THREE.SpriteMaterial( { map: Mark.texture, color: 0xffffff } );
